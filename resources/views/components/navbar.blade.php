...
<aside :class="{ 'left-[15px] max-[768px]:left-[10px]': isOpen, '-left-full': !isOpen }"
    ...
    @if (auth()->guard('admin')->check())
        <a href="{{ route('admin.dashboard') }}" class="fill-white flex items-center gap-2 p-2 text-3xl">
            <box-icon name='dashboard' type='solid'></box-icon>Dashboard
        </a>
    @elseif (auth()->guard('siswa')->check())
        <a href="{{ route('siswa.dashboard') }}" class="fill-white flex items-center gap-2 p-2 text-3xl">
            <box-icon name='dashboard' type='solid'></box-icon>Dashboard
        </a>
    @endif
    <ul class="h-[calc(100%-86px)]">
        @auth('admin')
            <li><a href="{{ route('admin.siswa') }}"
                    ...
                    <box-icon name='group' type='solid'></box-icon>Siswa</a>
            </li>
        ...
        <li><a href="{{ auth()->guard('admin')->check() ? route('admin.presensi') : route('siswa.presensi') }}"
                ...
                <box-icon name='calendar-check' type='solid'></box-icon>Presensi</a>
        </li>
        <li><a href="{{ auth()->guard('admin')->check() ? route('admin.peringkat') : route('siswa.peringkat') }}"
                ...
                <box-icon name='trophy' type='solid'></box-icon>Peringkat</a>
        </li>
    </ul>
    <ul>
        <li>
